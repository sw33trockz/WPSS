...
function update_notifier_menu() {
	...
	$theme_data =  wp_get_theme('RightNow'); // Get theme data from style.css (current version is what we want)

	if(version_compare($theme_data->Version, $xml->latest) == -1) {
		if(current_user_can('activate_plugins')){
		add_dashboard_page( $theme_data->Name . 'Theme Updates', 
							$theme_data->Name . '<span class="update-plugins count-1"><span class="update-count">New Update</span></span>', 
							'activate_plugins', 
							'rightnownupdates',
							'update_notifier');
		}
...
function update_notifier() {
	...
	<div class="wrap">
	    ...
	    <div id="message" class="updated below-h2"><p><strong>There is a new version of the <?php echo $theme_data->Name; ?> theme available.</strong> You have version <?php echo $theme_data->Name; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://renklibeyaz.com/rightnow.xml';
