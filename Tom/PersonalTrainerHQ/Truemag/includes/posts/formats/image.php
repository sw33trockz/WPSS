				...
				-------------------------------------------*/
					...
					if ( !empty( $st_['is_blog'] ) ) {
						...
						if ( $st_['secondary_sidebar'] == true ) {
							...
							echo '<a href="' . $st_['large_image_url'][0] . '" title="' . the_title_attribute( 'echo=0' ) . '">' . get_the_post_thumbnail( $post->ID, 'project-medium', ( function_exists( 'st_get_2x' ) ? st_get_2x( $post->ID, 'project-medium', 'array', 'size-original' ) : '' ) ) . '</a>'; }
			
						// if sidebar (-)
						else {

							echo get_the_post_thumbnail( $post->ID, 'archive-image', ( function_exists( 'st_get_2x' ) ? st_get_2x( $post->ID, 'archive-image' ) : '' ) ); }
