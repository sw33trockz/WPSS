...
function thrive_get_facebook_share( $post_link ) {
	$request  = wp_remote_get( "http://graph.facebook.com/?id=" . $post_link );
	$response = json_decode( wp_remote_retrieve_body( $request ) );

	return isset( $response->shares ) ? $response->shares : 0;
}
...
function _thrive_get_home_layout_ad_zones() {
	...
	$ad_zones = array();
	foreach ( $home_options_obj['blocks'] as $block ) {
		if ( $block['type'] == "ad" ) {
			$zone_key              = _thrive_get_custom_ad_zone_key( $block['ad_zone'] );
			$ad_zones[ $zone_key ] = $block['ad_zone'];
		}
	}
...
function _thrive_get_custom_ad_zone_key( $ad_zone_name ) {
	$ad_zone_key = "ad_zone_" . str_replace( " ", "_", trim( strtolower( $ad_zone_name ) ) );

	return $ad_zone_key;
}

function _thrive_filter_home_layout_options_array( $options_array ) {
	if ( ! is_array( $options_array ) ) {
		$options_array = array();
	}
	if ( ! isset( $options_array['blocks'] ) ) {
		$options_array['blocks'] = array();
	}
	if ( ! isset( $options_array['display_infinite'] ) ) {
		$options_array['display_infinite'] = 0;
	}
	if ( ! isset( $options_array['display_featured'] ) ) {
		$options_array['display_featured'] = 0;
	}
	if ( ! isset( $options_array['featured_custom'] ) ) {
		$options_array['featured_custom'] = 0;
	}
	if ( ! isset( $options_array['featured_custom_ids'] ) || ! is_array( $options_array['featured_custom_ids'] ) ) {
		$options_array['featured_custom_ids'] = array();
	}
	if ( ! isset( $options_array['display_infinite_method'] ) ) {
		$options_array['display_infinite_method'] = 0;
	}
	$options_array['selected_featured_custom'] = array();
	foreach ( $options_array['featured_custom_ids'] as $id ) {
		$options_array['selected_featured_custom'][] = array(
			'id'   => $id,
...
function _thrive_get_ad_group_locations() {
	$locations = array(
		'in_content'      => __( "In Content", 'thrive' ),
		'header'          => __( "Header", 'thrive' ),
		'blog_or_index'   => __( "Blog or Index View", 'thrive' ),
		'widget_standard' => __( "Widget Standard", 'thrive' ),
		'widget_buttons'  => __( "Widget Buttons", 'thrive' ),
		'slideshow'       => __( "Slideshow", 'thrive' ),
		//'site_bg' => __("Site Background", 'thrive'),
	);

	$ad_zones = _thrive_get_home_layout_ad_zones();

	foreach ( $ad_zones as $key => $name ) {
		$locations[ $key ] = "Ad Zone - " . $name;
	}

	return $locations;
}

function _thrive_get_ad_group_targets() {
	$default_targets = array(
		'post' => __( "Posts", 'thrive' ),
		'page' => __( "Pages", 'thrive' ),
		'blog'  => __( "Post list", 'thrive' ),
	);

	return $default_targets;
}

function _thrive_get_ad_sizes_list() {
	$sizes = array(
		"300x250",
		"338x280",
		"728x90",
...
function _thrive_get_mobile_ad_sizes_list() {
	$sizes = array(
		"300x250",
		"338x280",
		"320x50",
...
function _thrive_get_category_custom_colors() {
	$colors = array(
		'Default' => 'def',
		'Blue'    => 'blue',
		'Green'   => 'green',
...
function _thrive_get_single_page_related_posts( $postId, $offset = 0, $limit = 10 ) {
	$relatedPostsStr = get_post_meta( $postId, "_thrive_meta_related_posts_list", true );
	$items_array     = array();
	$excludePosts    = array( $postId );

	if ( $relatedPostsStr && $relatedPostsIds = json_decode( $relatedPostsStr ) ) {
		$relatedPostsIds = array_slice( $relatedPostsIds, $offset, $limit );
		foreach ( $relatedPostsIds as $pid ) {
			$p = get_post( $pid );
			array_push( $items_array, $p );
			$excludePosts[] = $pid;
		}
	}

	if ( count( $items_array ) < $limit ) {
		$related_args = array(
	...
	$items_array = array_slice( $items_array, $offset, $limit );

	return $items_array;
}

function _thrive_get_homepage_layout_obj() {
	$home_options_str = thrive_get_theme_options( "homepage_layout_settings" );
	if ( ! is_array( $home_options_str ) ) {
		$home_options_str = json_decode( $home_options_str, true );
	}
...
function _thrive_get_single_page_latest_posts( $postId, $offset = 0, $excludePosts = array() ) {
	$tempItems    = array();
	$relatedPosts = _thrive_get_single_page_related_posts( $postId, $offset );
	$tempItems    = array_slice( $relatedPosts, $offset, 10 );

	if ( count( $tempItems ) < 10 ) {
		foreach ( $tempItems as $item ) {
			$excludePosts[] = $item->ID;
		}
		$args = array(
			'posts_per_page' => 10 - count( $tempItems ),
			'offset'         => $offset,
			'post__not_in'   => $excludePosts,
		);

		$tempItems = array_merge( $tempItems, get_posts( $args ) );
	}

	return $tempItems;
}

function _thrive_check_focus_area_template_position( $focusAreaId, $position = "bottom" ) {

	$template = get_post_meta( $focusAreaId, "_thrive_meta_focus_template", true );
	if ( ! $template ) {
		return false;
	}
