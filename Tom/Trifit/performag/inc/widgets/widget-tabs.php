...
class Thrive_Tabs_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$section_class = ( isset( $args['id'] ) && ( $args['id'] == "sidebar-1" || $args['id'] == "sidebar-2" ) ) ? "widget tabs_widget" : "widget tabs_widget cols left";
		$txt_class     = ( $instance['thumbnail_size'] == "on" ) ? "" : " noImageTab";
		?>
		<section class="widget tabs_widget" id="<?php echo $this->id; ?>">
			<div class="scT">

				<ul class="tabs clearfix scT-tab">
					<?php $tab_count = count( $tabs_contents['labels'] ); ?>
					<?php foreach ( $tabs_contents['labels'] as $key => $label ): ?>
						<li style="width: <?php echo 100 / $tab_count; ?>%"
						    class="<?php if ( $key == 0 ): ?>tS<?php endif; ?>">
							<a class="<?php if ( $key == 0 ): ?>active_tab<?php endif; ?> tab_widget_item">
								<?php echo $label; ?>
							</a>
						</li>
					<?php endforeach; ?>
										...
										<?php if ( $instance['thumbnail_size'] && $instance['thumbnail_size'] == "on" ): ?>
											<div class="left tim default_tab_image">
												...
												<?php else: ?>
													<a href="<?php echo $c['url']; ?>">
														<div class="wti"
														     style="background-image: url('<?php echo get_template_directory_uri(); ?>/images/tabs_default_image.png')"></div>
													</a>
												<?php endif; ?>
											</div>
										<?php endif; ?>
										<div class="left txt<?php echo $txt_class; ?>">
											...
											<i>
												<?php if ( ( $key == "popular" || $key == "trending" ) && isset( $c['no_comments'] ) && $instance['display_no_comments'] != "off" ): ?>
													<span><?php echo $c['no_comments']; ?></span>
		...
		if ( $show_tabs_menu == "on" && $instance['menu'] > 0 ) {
			...
			foreach ( (array) $menu_items as $key => $menu_item ) {
				array_push( $items_array, array(
					...
					'image' => $this->_get_featured_image( $menu_item->object_id )
				) );
			}
			$items_array      = array_slice( $items_array, 0, $limit_content );
