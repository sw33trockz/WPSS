...
function thrive_theme_options_init() {
	...
	add_settings_section( 'related_box_settings', __( ' ', 'thrive' ), '__return_false', 'theme_related_box_options' );
	...
	add_settings_section( 'magazine_settings', __( "Magazine Features", 'thrive' ), '__return_false', 'theme_magazine_options' );
	add_settings_section( 'magazine_home_settings', "", '__return_false', 'theme_magazine_home_options' );
	...
	add_settings_field( 'color_theme', $options_titles['color_theme'], 'thrive_settings_field_color_theme', 'theme_style_options', 'style_settings', $options );
	...
	add_settings_field( 'enable_infinite_scroll', $options_titles['enable_infinite_scroll'], 'thrive_settings_field_enable_infinite_scroll', 'theme_magazine_options', 'magazine_settings', isset( $options['enable_infinite_scroll'] ) ? $options['enable_infinite_scroll'] : "off" );
	add_settings_field( 'enable_infinite_scroll_single', $options_titles['enable_infinite_scroll_single'], 'thrive_settings_field_enable_infinite_scroll_single', 'theme_magazine_options', 'magazine_settings', isset( $options['enable_infinite_scroll_single'] ) ? $options['enable_infinite_scroll_single'] : "off" );
	add_settings_field( 'enable_home_custom_layout', $options_titles['enable_home_custom_layout'], 'thrive_settings_field_enable_home_custom_layout', 'theme_magazine_home_options', 'magazine_home_settings', isset( $options['enable_home_custom_layout'] ) ? $options['enable_home_custom_layout'] : "off" );
	add_settings_field( 'enable_share_buttons_on_blog', $options_titles['enable_share_buttons_on_blog'], 'thrive_settings_field_enable_share_buttons_on_blog', 'theme_magazine_options', 'magazine_settings', isset( $options['enable_share_buttons_on_blog'] ) ? $options['enable_share_buttons_on_blog'] : "on" );

	add_settings_section( 'social_settings', __( 'Social header links', 'thrive' ), '__return_false', 'theme_social_options' );
...
function thrive_get_theme_options( $key = null, $postId = 0 ) {
	...
	/*
	  if (!is_singular() && strpos($options['social_display_location'], "list") !== false) {
	  $options['enable_social_buttons'] = 1;
	  } else {
...
function thrive_theme_options_render_page() {
	...
	$add_ad_groups = get_posts( array( 'post_type' => 'thrive_ad_group' ) );

	$all_categories = get_categories();

	$home_options_str = thrive_get_theme_options( "homepage_layout_settings" );
	if ( ! is_array( $home_options_str ) ) {
		$home_options_str = json_decode( $home_options_str, true );
	}
	$home_options_obj  = _thrive_filter_home_layout_options_array( $home_options_str );
	$home_options_page = thrive_get_theme_options( "homepage_layout_page" );

	wp_enqueue_script( 'thrive-home-layout' );

	$js_params_array2 = array(
		'saveLayoutUrl'            => "zz",
		'noonce'                   => $wpnonce,
		'featured_custom_ids'      => $home_options_obj['featured_custom_ids'],
		'selected_featured_custom' => $home_options_obj['selected_featured_custom'],
	);

	wp_localize_script( 'thrive-home-layout', 'ThriveHomeLayout', $js_params_array2 );
...
function thrive_settings_field_social_display_location( $options ) {
	...
	$socialDisplayLocationExploded = explode( ',', $options['social_display_location'] );
	$checked_posts                 = $checked_pages = $checked_posts_top = $checked_pages_top = $checked_list = '';

	$value_hidden = "";
	//keep the values separated by commas in a hidden input
	if ( in_array( 'posts', $socialDisplayLocationExploded ) ) {
		$value_hidden .= "posts,";
		$checked_posts = " checked";
	}
	if ( in_array( 'pages', $socialDisplayLocationExploded ) ) {
		$value_hidden .= "pages,";
		$checked_pages = " checked";
	}
	if ( in_array( 'posts_top', $socialDisplayLocationExploded ) ) {
		$value_hidden .= "posts_top,";
		$checked_posts_top = " checked";
	}
	if ( in_array( 'pages_top', $socialDisplayLocationExploded ) ) {
		$value_hidden .= "pages_top,";
		$checked_pages_top = " checked";
	}
	if ( in_array( 'list', $socialDisplayLocationExploded ) ) {
		$value_hidden .= "list,";
		$checked_list = " checked";
	}

	echo "<input class='tt-chk-social-display-location' value='posts' $checked_posts type='checkbox'/>" . __( "Posts", 'thrive' ) . " ";
	echo "<input class='tt-chk-social-display-location' value='pages' $checked_pages type='checkbox' />" . __( "Pages", 'thrive' ) . " ";
	echo "<input class='tt-chk-social-display-location' value='posts_top' $checked_posts_top type='checkbox' />" . __( "Top of posts", 'thrive' ) . " ";
	echo "<input class='tt-chk-social-display-location' value='pages_top' $checked_pages_top type='checkbox' />" . __( "Top of pages", 'thrive' ) . " ";
	echo "<input class='tt-chk-social-display-location' value='list' $checked_list type='checkbox' />" . __( "Post list", 'thrive' ) . " ";
...
function thrive_settings_field_social_site_image( $options ) {
	...
	echo '<input type="button" class="thrive-select-image thrive_options pure-button upload" id="thrive_meta_social_button_upload" value="Upload"/>
