...
ThriveWidgetsOptions.bind_handlers = function (callType) {
    ...
    if (callType != 'ajax') {
        setTimeout(function () {
            jQuery(".widget-liquid-right select.tt-widget-ad-sel-groups:not(.select2-offscreen)").on("change", function (e) {
                var tempAdValues = JSON.stringify(jQuery(this).val());
                jQuery(this).parents("p").find(".tt-hidden-sel-ad-groups").val(tempAdValues);
            }).select2();
        }, 100);
    } else {
        jQuery(".widget-liquid-right select.tt-widget-ad-sel-groups:not(.select2-offscreen)").on("change", function (e) {
...
jQuery(document).ready(function () {
    ...
    ThriveWidgetsOptions.bind_handlers('load');

    jQuery(document).ajaxSuccess(function (e, xhr, settings) {
        ThriveWidgetsOptions.bind_handlers('ajax');
    });

    jQuery('div.widgets-sortables').bind('sortupdate', function (event, ui) {
        ThriveWidgetsOptions.bind_handlers('sortable');
    });
});
