		...
		if ( ! empty( $item ) ) {
			if ( $attr['thumbnails'] == "on" ) {
				...
				if ( $featured_img ) {
					...
					$output .= "<a href='" . $item->url . "' style='background-image: url(\"" . get_template_directory_uri() . "/images/post_default.jpg\")'></a></div>";
				}
...
function thrive_shortcode_custom_phone( $attr, $content ) {
	...
	$output = "<div class='phone " . $attr['color'] . "'><div class='phr'><span class='fphr'>";
	$output .= $attr['phone_text'];
	$output .= "</span>";
	$output .= "<span class='mphr'>";
	$output .= $attr['mobile_phone_text'];
	$output .= "</span>";
	$output .= " <a href='tel:" . $attr['phone_no'] . "'>" . $attr['phone_no'] . "</a>";
	$output .= "</div></div>";
...
function thrive_shortcode_text_block( $attr, $content ) {
	...
	if ( empty( $attr['headline'] ) ) {
		...
		$output = "<div class='clear'></div><div class='thrivecb " . $attr['color'] . "'><div class='" . $colour_scheme . " cbt'><h3>" . $attr['headline'] . "</h3></div><div class='" . $colour_scheme . "'>" . do_shortcode( $content ) . "</div></div>";
	}
...
function thrive_shortcode_toggle( $attr, $content ) {
	...
	$output = "<div class='faq'><div class='faqI'><div class='faqB'><h4><span></span>" . $attr['title'] . "</h4></div><div class='faqC'><p>" . do_shortcode( $content ) . "</p></div></div></div>";
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	$output .= "<div class='scbgi'>";
	foreach ( $posts as $p ) {
		$featured_img_data = thrive_get_post_featured_image( $p->ID, "tt_post_gallery" );
		$featured_img      = $featured_img_data['image_src'];
		...
		$output .= "</a>";
		$output .= "<div class='scbt'>";
		$categories = get_the_category( $p->ID );
		$cat        = isset( $categories[0] ) ? $categories[0] : null;
		if ( $cat ) {
			$output .= "<div class='cat_b " . _thrive_get_meta_category_class( $cat->term_id ) . "'><a href='" . get_category_link( $cat->term_id ) . "'>" . $cat->cat_name . "</a></div>";
		}
		$output .= "<a href='" . get_permalink( $p->ID ) . "'><h5>" . get_the_title( $p->ID ) . "</h5></a>";

		$author_info = _thrive_get_author_info( $p->post_author );
		$output .= "<span class='scd'>" . date( get_option( 'date_format' ), strtotime( $p->post_date ) ) . "  /  " . __( "By", 'thrive' ) . " " . $author_info['display_name'] . "</span>";
	...
	foreach ( $posts as $p ) {
		if ( $attr['thumbnails'] == "on" ) {
			...
			if ( $featured_img && isset( $featured_img[0] ) ) {
				...
				$output .= "<a href='" . get_permalink( $p->ID ) . "' style='background-image: url(\"" . get_template_directory_uri() . "/images/post_default.jpg\")'></a></div>";
			}
			$output .= "<div class='left txt'>";
...
function thrive_shortcode_testimonal( $attr, $content ) {
	...
	$output = "<div class='sctm'><div class='tst'>";
	if ( ! empty( $attr['image'] ) ) {
		$output .= "<div class='tst-i' style='background-image: url(\"" . $attr['image'] . "\")'></div>";
	} else {
		$output .= "<div class='tst-i' style='background-image: url(" . get_template_directory_uri() . "/images/testimonial.jpg)'></div>";
	}
	$output .= "</div><div class='bts'><p>" . $content . "</p><span>" . $attr['name'] . $attr['company'] . "</span></div></div>";
...
function thrive_shortcode_fill_counter( $attr, $content ) {
	...
	$output = '<div class="fill-counter nsd ' . $attr['color'] . '">
                    <div class="fill-counter-inner" style="stroke-dasharray: ' . ( $attr['percentage'] * 6.28 ) . ' 628;">
                        <svg viewBox="0 0 200 200" class="fill-counter-circle" shape-rendering="optimizeSpeed">
	...
	$output = '
        <div class="cdt nsd sns ' . $attr['color'] . '" data-date="' . $date . '" data-fade="' . $attr['fade'] . '" data-message="' . ( $attr['text'] == '' ? '0' : '1' ) . '" data-server-now="' . $now->format( 'Y-m-d\TH:i:sP' ) . '">
            ...
            <div class="cdto">
                ...
                <span class="cdtl">' . __( "Days", 'thrive' ) . '</span>
            </div>
            <div class="cdto">
                <div class="cdtc hour">
                    <div class="cdfc"></div>
                </div>
                <span class="cdtl">' . __( "Hours", 'thrive' ) . '</span>
            </div>
            <div class="cdto">
                <div class="cdtc minute">
                    <div class="cdfc"></div>
                </div>
                <span class="cdtl">' . __( "Minutes", 'thrive' ) . '</span>
            </div>
            <div class="cdto">
                <div class="cdtc second">
                    <div class="cdfc"></div>
                </div>
                <span class="cdtl">' . __( "Seconds", 'thrive' ) . '</span>
            </div>
            </div>
            </div>
